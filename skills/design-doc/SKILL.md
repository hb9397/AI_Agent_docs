...
> 복수 애플리케이션인 경우 기존 `*-DESIGN.md` 파일 존재 여부도 확인하여, 신규 작성인지 갱신인지 판단한다.
...
스케일 확인 후, 참고할 기존 설계 문서(PRD, 기획서, DB 설계서, 관련 코드 등)가 있는지 묻는다.

> "참고할 기존 문서나 코드가 있으면 파일 또는 텍스트로 공유해 주세요. 특히 DB 설계가 이미 있다면 함께 주시면 좋겠습니다. 없으면 바로 인터뷰를 시작합니다."
...
저장 승인 시:
- **단일 앱**: 기본 경로 `{project}/.docs/DESIGN.md`
- **복수 앱**: 기본 경로 `{project}/.docs/{앱 디렉토리명}-DESIGN.md`
- `.docs/` 디렉토리가 없으면 생성한다.
