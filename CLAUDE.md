...
3. **C-2 경로 표준** — `.docs/` 경로는 단일/복수에 따라 분기한다 (상세: `리팩토링 작업 계획서.md` §3-2).
...
- `Harness_Engineering.md`와 `Harness_Engineering_Intro.md`는 스킬 변경 시 함께 업데이트한다.
- 커밋 메시지는 Conventional Commits 규격을 따른다.

## 참조 문서

- [Harness_Engineering.md](./Harness_Engineering.md) — 스킬 맵, 단일 통합 흐름, 데이터 계약
- [Harness_Engineering_Intro.md](./Harness_Engineering_Intro.md) — 도입 배경, 철학, 사용 예시
- [리팩토링 작업 계획서.md](./리팩토링%20작업%20계획서.md) — D-1~D-7 의사결정, C-1/C-2/C-3 공통 규약
- [Agent_Skills_Repo_Structure_Analysis.md](./Agent_Skills_Repo_Structure_Analysis.md) — 타 AI 하네스 비교 분석
