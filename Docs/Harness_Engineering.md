...
> - `.docs/.gitignore` — 로컬 전용(미추적) 영역 지정. 단일 앱에선 소스 레포의 중첩 `.gitignore`, 복수 앱에선 `.docs` 레포의 루트 `.gitignore`로 동작.
> - `.docs/_inbox/` — 에이전트에게 읽힐 파일을 잠시 올려두는 **로컬 전용** 공간. 내용은 git에 올라가지 않고 `.gitkeep`·`README.md`만 추적되어 폴더 구조만 공유된다.
...
>
> **로드맵 인덱스 문서**: 두 스킬은 같은 디렉토리 안에 `{YYMMDD}-0.{앱이름}-roadmap-impl-index.md` 파일 1개를 공용으로 유지한다. 그 디렉토리에 처음 impl 문서를 만들 때 함께 생성하고(파일명의 날짜는 최초 생성 시점에 고정, 이후 rename하지 않음), 이후 다른 impl 문서가 새로 생기거나 갱신될 때마다 분할 구조 표·현재 진행 위치·갱신일자를 함께 갱신한다. 참고 예시: `.docs/be-keai-collector/impl-doc/lhb9397/260629-2.collector-roadmap-impl-index.md`.
