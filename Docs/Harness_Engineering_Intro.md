...
결국 AI를 썼는데도 팀의 코드베이스는 더 빠르게 일관성을 잃는다.

이 문서는 그 문제를 해결하기 위해 작성되었다.  
핵심은 단순하다.

> AI를 잘 쓰는 개인을 만드는 것이 아니라,  
> 서로 다른 사람과 서로 다른 모델이 써도 결과가 크게 흔들리지 않는 공통 작업 기준을 만드는 것.
...
좋은 결과가 나와도 그것이 **사람의 경험** 때문인지, **프롬프트** 때문인지, **문서 구조** 때문인지 구분하기 어렵다.
...
이 저장소가 하려는 일은 "좋은 프롬프트 몇 줄"을 모아두는 것이 아니다.
대신 다음을 고정한다.

1. 설계 문서의 형태
2. 에이전트가 참조할 컨텍스트 문서의 형태
3. 구현 지침서의 형태
4. 리뷰, 검사, 커밋, 동기화 흐름
5. 하나의 기본 흐름 — RFP·레거시·아이디어 어디서든 같은 흐름으로 합류

즉, AI에게 일을 "잘 시키는 말버릇"이 아니라,
**AI가 같은 기준으로 일하게 만드는 작업 구조**를 만든다.
...
루트 가이드 문서(`CLAUDE.md`, `AGENTS.md`)와 주제별 `.docs/instruction/*-instruction.md`(아키텍처·코드 스타일·프레임워크·API·통신·파일 생성·에이전트 전용 규칙)로 프로젝트 규칙과 아키텍처를 고정한다.

단일 앱에서는 프로젝트 루트에 `CLAUDE.md` + `.docs/instruction/`이 위치한다. 복수 앱에서는 앱별 컨텍스트가 `.docs/{앱}-context.md` + `.docs/{앱}/instruction/`으로 분리되고, 프로젝트 전체를 아우르는 루트 문서는 `.docs/root-context/CLAUDE.md`에 복사본을 보관한다.
...
| 스킬                | 역할                                                                    | 언제 쓰는가                                        |
| ------------------- | ----------------------------------------------------------------------- | -------------------------------------------------- |
| `harness-setup`     | 원본 하네스 저장소에서 스킬·문서를 프로젝트에 설치·업데이트 + git으로 관리하지 않는 루트 파일 관리 + `.docs/` 안내·정책 파일(`README.md`·`.gitignore`·로컬 전용 `_inbox/`) 생성 | 프로젝트에 하네스를 처음 설치하거나 원본을 업데이트할 때 |
| `rfp-ingest`        | RFP에서 특정 SFR을 뽑아 해석하고 대화 컨텍스트로 전달                    | RFP 기반 프로젝트를 시작할 때                      |
| `design-doc`        | 인터뷰를 통해 구조화된 설계 문서 생성                                   | 아이디어를 실제 작업 문서로 바꿀 때                |
| `context-doc`       | 얇은 루트 가이드 문서(`CLAUDE.md`/`AGENTS.md`) + 주제별 `.docs/instruction/*` 생성 | 설계가 끝나고 에이전트가 계속 참고할 기준이 필요할 때 |
...
- 프로젝트에 하네스를 처음 설치하거나 갱신한다 → `harness-setup`
- 요구사항을 해석하는 중이다 → `rfp-ingest` 또는 `design-doc`
- 설계를 에이전트 규칙으로 고정하고 싶다 → `context-doc`
...
0. **하네스 설치**: `/harness-setup`으로 원본 하네스 저장소에서 스킬·문서를 프로젝트에 설치 (최초 1회 또는 업데이트 시)
...
이 방식이 효과적인 이유는 AI에게 아래 네 가지를 동시에 주기 때문이다.
...
안정적인 결과는 보통 문서에서 나온다.

- 설계 문서
- `CLAUDE.md`
- `.docs/instruction/*-instruction.md` (아키텍처·코드스타일·프레임워크 등 7종)
- 구현 지침서
- RFP 분석 결과
...
구조가 바뀌었는데 `CLAUDE.md`가 예전 상태면, 다음 에이전트는 낡은 규칙을 믿고 또 잘못 구현한다.
...
AI를 각자 쓰면 결과는 빨라질 수 있지만, 팀의 기준은 쉽게 흩어진다.
