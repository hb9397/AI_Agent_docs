...
> 대상 레포: `AI_Agent_docs`
> 기준 문서: `README.md`, `Harness_Engineering_v2.md`, `Harness_Engineering_Intro.md`, `skills/*/SKILL.md`, 핵심 `prompts/` 및 `templates/`

---

## 검토 요청 원문

이 문서는 아래 요청을 기준으로 작성한 평가 문서다.
다른 에이전트는 아래 질의 의도와 평가 축을 먼저 이해한 뒤 본문을 읽어야 한다.

> 현재 레포지토리의 스킬 파일과
> `README.md`, `Harness_Engineering_v2.md`, `Harness_Engineering_Intro.md`를 꼼꼼하게 읽어보고,
>
> 제안요청서 문서가 있던 없던 상관없이 모든 경우에 잘 적용할 수 있는지?
> 두 경우 잘 가이드 및 연계 되는지?
> 프레임워크 -> ex> Spring boot, NextJS, FastAPI, .NET ..... 등 에 상관없이 모두 범용적으로 적용되는지
> 일부 아키텍쳐에 종속적이지 않는지?
>
> 위 4가지 경우를 비롯한 범용적 측면에서 인터뷰를 통해 모든 상황에 유연하게 대처 가능한지 점검해줘
>
> 하네스 엔지니어링 전문가 또는 바이브 코딩 전문가 또는 스킬 전문가 또는 클로드를 개잘쓰는 사람 입장에서
> `skills/skill-design`, `$skill-creator` 등의 지식을 활용해서
> 현재 레포지토리에 대한 분석 및 평가좀

정리하면, 이 문서는 아래 5개 축을 중심으로 평가한다.

1. 제안요청서 문서 유무와 무관한 적용 가능성
2. 문서가 있는 경우와 없는 경우의 가이드 및 연계 품질
3. 프레임워크 비종속성
4. 아키텍처 비종속성
5. 인터뷰 기반 대응 유연성
...
`impl-doc`은 BE 단일 기능과 FE 단일 기능까지 흡수하는 단일·소규모 범용 작업 폴백으로 확장됐다.
...
**판정: 개념적으로 좋음, 운영 문구는 지속 점검 필요**

좋은 점:

- `Harness_Engineering_v2.md`는 `design-doc OUTPUT_V2`를 중심 허브로 놓고 `context-doc`, `impl-fe-be-doc`, `impl-doc`, `impl-reuse-scan`, `impl-verify`, `design-prototype-docs`로 이어지는 데이터 계약을 비교적 잘 설명한다.
- 전체 플로우 관점에서는 `아이디어 기반`, `RFP 기반`, `레거시 부트스트랩`이 하나의 하네스로 수렴하도록 설계돼 있다.

문제점:

- `context-doc`의 실제 산출물은 `CLAUDE.md + AGENTS.md + .instruction/*-instruction.md`이므로 문서 전반에서 이 표현을 유지해야 한다.
- 다중 화면·페어 다중 기능은 `impl-fe-be-doc`, 단일·소규모 범용 작업은 `impl-doc`로 계속 분리해서 설명해야 한다.
- 레거시 산출물명이나 오래된 스킬 분기 표현이 다시 유입되지 않도록 주기적으로 점검해야 한다.
...
- `impl-doc`는 CLI, 스크립트, 라이브러리, 단독 백엔드, ETL, AI Agent 도구, 인프라 자동화, BE 단일 기능(엔드포인트 1~수개), FE 단일 기능(컴포넌트·훅·화면 1개)까지 수용하도록 설계돼 있다.
...
- RFP/SFR 화면 중심 설계는 `impl-fe-be-doc` 화면 중심 모드로 흡수되어 웹 UI 축을 이룬다.
...
- BE 단일 기능(엔드포인트 1~수개)과 FE 단일 기능(컴포넌트·훅·화면 1개)도 `impl-doc`으로 흡수할 수 있어 소규모 작업 분기가 더 명확해졌다.
...
- 레거시 instruction 파일명 표현 재유입 가능성
- `impl-doc`와 `impl-fe-be-doc` 설명 혼선 → 분기 기준이 '다중 화면·페어 다중 기능 여부'로 통일됨
...
`skills/skill-design/prompts/design-principles.md`는
...
`impl-reuse-scan`의 구현 직전 발견·보고(자동 반영 금지)와 `impl-verify`의 Phase 종료 검증 매트릭스 산출(코드/지침서 수정 금지)도 새 강점으로 볼 수 있다.
...
- `skills/design-doc/prompts/interview.md`
- `skills/design-doc/prompts/scale-routing.md`
- `skills/design-doc/templates/INPUT_V2.md`
- `skills/design-doc/templates/OUTPUT_V2.md`
- `skills/rfp-ingest/SKILL.md`
- `skills/rfp-ingest/prompts/clarification.md`
- `skills/context-doc/SKILL.md`
- `skills/harness-bootstrap/SKILL.md`
- `skills/harness-bootstrap/prompts/stack-detection.md`
- `skills/impl-fe-be-doc/SKILL.md`
- `skills/impl-doc/SKILL.md`
- `skills/impl-reuse-scan/SKILL.md`
- `skills/impl-verify/SKILL.md`
- `skills/design-prototype-docs/SKILL.md`
- `skills/create-prototype/SKILL.md`
- `skills/frontend-design/SKILL.md`
- `skills/skill-design/prompts/design-principles.md`
