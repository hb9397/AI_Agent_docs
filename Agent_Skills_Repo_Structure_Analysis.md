...
| 방식 | 지원 여부 | 사용법 |
|------|-----------|--------|
| Claude Code 직접 설치 | 지원 | `~/.claude/skills/gstack`에 clone 후 `./setup` |
| Claude Code team mode | 지원 | `./setup --team` 및 `gstack-team-init required/optional` |
| Codex host 설치 | 지원 | `./setup --host codex`, 결과는 `~/.codex/skills/gstack-*/` |
...
- 운영/학습: `retro`, `investigate`, `learn`, `context-save`, `context-restore`
