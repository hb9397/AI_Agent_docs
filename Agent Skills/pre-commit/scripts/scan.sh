...
scan_code "catch.*{}" "$TARGET" 2>/dev/null
grep -rn --include="*.py" "except.*:\s*$" "$TARGET" 2>/dev/null
scan_code "catch\s*{" "$TARGET" 2>/dev/null | head -20
echo "(위 결과 중 빈 블록 확인 필요)"
echo ""

echo "### 에러 무시 주석 (// ignore, # noqa 등)"
scan_code -i "// *ignore\|# *ignore\|# *noqa" "$TARGET" 2>/dev/null || echo "(없음)"
echo ""

# --- 2. 타임아웃 ---
echo "## 2. 외부 호출 (타임아웃 확인 필요)"
echo ""
scan_code "fetch(\|axios\.\|requests\.\|HttpClient\|http\.Get\|http\.Post" "$TARGET" 2>/dev/null | head -20 || echo "(없음)"
echo ""
...
scan_code -iE "(password|apikey|api_key|secret|token)\s*[:=]\s*[\"']" "$TARGET" 2>/dev/null | grep -iv "test\|mock\|example\|placeholder\|TODO\|env\." | head -20 || echo "(없음)"
echo ""
...
scan_code "TODO\|FIXME\|HACK\|XXX" "$TARGET" 2>/dev/null | head -20 || echo "(없음)"
echo ""

echo "### 기한 없는 TODO"
scan_code "TODO" "$TARGET" 2>/dev/null | grep -v "TODO(@\|TODO(\|#[0-9]" | head -20 || echo "(없음)"
echo ""
