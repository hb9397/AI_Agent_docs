...
`/create-prototype` 명령이 들어오면 아래 4단계를 **순서대로** 진행한다.
이미 제공된 정보는 건너뛰되, 빠진 정보는 반드시 사용자에게 요청한다.
...
3. `examples/SFR-018.html` — 실제 완성된 프로토타입 예제 (코드 스타일, 구조, 컴포넌트 패턴의 기준)
...
분리 구조로 결정됐을 때 **Agent 툴이 사용 가능한 환경이면 병렬 subagent로 생성**한다.
...
| 환경 | 처리 방식 |
|---|---|
| Claude Code, Codex (Agent 툴 가능) | 병렬 subagent로 동시 생성 |
| Claude.ai, 기타 (Agent 툴 없음) | 메인 파일 → 조각 파일 순서대로 순차 생성 |

Agent 툴 호출이 실패하거나 미지원 환경이면 **순차 생성으로 자동 전환**한다.
