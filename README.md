...
```
AI_Agent_docs/
├── Agent Skills/   ← IDE Agent가 직접 호출하는 스킬과 템플릿의 단일 소스
└── example/        ← 산출물 예시
```
...
| 스킬 | 트리거 예시 | 적합한 경우 |
|------|------------|------------|
| `impl-fe-be-doc` | `작업지침서 만들어줘`, `Phase 나눠줘`, `화면별 구현 지침` | 일반 웹앱, FE/BE 역할 분리 병행 개발, RFP/SFR 화면 중심 구현 |
| `impl-screen-doc` | `화면별 구현 지침`, `SFR 화면 구현` | 레거시 분리형 화면 지침. 신규 흐름은 `impl-fe-be-doc` 화면 중심 모드 권장 |
