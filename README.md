...
- **단일 애플리케이션 프로젝트**(git repo 1개)와 **복수 애플리케이션 프로젝트**(사용자가 직접 만든 컨테이너 폴더 아래 `.docs`·각 앱·하네스 소스 레포가 각각 독립 git)를 모두 지원한다.
- 산출물 생성 또는 코드·커밋 적용범위를 갖는 스킬은 STEP 0에서 **C-1 게이트**(프로젝트 유형 감지 + 사용자 확인)를 거친다. 프로젝트 유형에 따라 산출물 경로가 분기된다.
