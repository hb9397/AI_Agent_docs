...
> 현재 저장소 기준 운영 가이드: [`Harness_Engineering.md`](./Harness_Engineering.md)

> 타 AI 하네스 비교·분석: [`Agent_Skills_Repo_Structure_Analysis.md`](./Agent_Skills_Repo_Structure_Analysis.md)

---

## 대원칙

- **단일 애플리케이션 프로젝트**(git repo 1개)와 **복수 애플리케이션 프로젝트**(사용자가 직접 만든 컨테이너 폴더 아래 `.docs`·각 앱·하네스 소스 레포가 각각 독립 git)를 모든 스킬이 동일하게 지원한다.
- 모든 산출물·적용범위 스킬은 STEP 0에서 **C-1 게이트**(프로젝트 유형 감지 + 사용자 확인)를 거친다.
- `skills/`가 스킬·프롬프트·템플릿의 **단일 원천(source of truth)**이며, 프로젝트에는 `harness-setup`으로 설치한다.
...
# 2. 프로젝트 폴더에서 하네스 설치 또는 갱신
/harness-setup
```

`harness-setup`은 현재 폴더 구조를 탐색한 뒤 단일/복수 애플리케이션 프로젝트 여부와 적용 대상을 사용자에게 확인한다. 승인 후 `.claude/skills`·`.agents/skills`에 스킬을 설치하고, 프로젝트 유형에 맞는 `.docs/` 구조와 `CLAUDE.md`/`AGENTS.md`를 준비한다. 이후 정본 레포가 업데이트되면 같은 명령으로 다시 실행해 배포본을 갱신한다.
...
│   └── src/                      ← 소스코드 구조
└── keai-AI-Harness-docs/         ← 이 하네스 정본 레포 (별도 git 관리)
```

단일 앱에서 git으로 관리되는 대상:

- `keai-portal/`: 소스코드, `.docs/*`, `CLAUDE.md`, `AGENTS.md`, `.claude/skills`, `.agents/skills`
- `keai-AI-Harness-docs/`: 정본 `skills/`, 운영 문서, 템플릿

### 복수 애플리케이션 프로젝트 예시

복수 앱은 프로젝트 최상위 폴더 자체를 git으로 관리하지 않는다. 각 애플리케이션과 하네스 정본, `.docs` 저장소를 각각 독립 레포로 관리한다.

```text
keai/                             ← 사용자 생성 컨테이너 폴더 (git 관리 안 함)
...
│   └── src/                      ← 소스코드 구조
├── keai-AI-Harness-docs/         ← 이 하네스 정본 레포 (별도 git 관리)
├── .docs/                        ← 프로젝트 전체 AI 문서 저장소 (별도 git 관리 권장)
...
- `keai-AI-Harness-docs/`: 정본 `skills/`, 운영 문서, 템플릿
- `.docs/`: 프로젝트 전체 AI 문서와 앱별 컨텍스트 문서. 팀 공유가 필요하면 별도 GitHub/GitLab/Gitea 레포로 관리
...
이 루트 미관리 파일들은 `harness-setup`이 정본 레포와 `.docs/root-context/`를 기준으로 다시 만들거나 갱신한다.
...
이 저장소는 스킬의 단일 원천이다. 스킬을 고칠 때는 먼저 `{프로젝트명}-AI-Harness-docs/skills/{스킬명}/`을 수정하고 커밋한다.

```text
{프로젝트명}-AI-Harness-docs/skills/*       ← 정본
        │
        └─ /harness-setup 재실행
...
`.claude/skills`와 `.agents/skills`는 배포본이다. 두 경로 간 횡적 불일치만 맞출 때는 `agent-sync`를 사용하고, 정본 레포의 최신 내용을 프로젝트에 가져올 때는 `harness-setup`을 사용한다.
...
```
AI_Agent_docs/                   ← 정본 레포(source of truth)
├── skills/                      ← 모든 스킬의 단일 소스
...
| 스킬 | 트리거 예시 | 역할 |
|------|------------|------|
| `harness-setup` | `하네스 설치`, `스킬 업데이트` | 정본 레포에서 스킬·문서를 프로젝트에 설치·업데이트 + 루트 미관리 파일 관리 |
...
| `agent-sync` | `동기화해줘`, `sync` | CLAUDE.md / AGENTS.md / Skills 등 Agent 관련 문서 횡적(lateral) 동기화 (정본 pull은 harness-setup 담당) |
...
현재 저장소 기준 스킬 목록, 단일 통합 워크플로우(RFP·아이디어·레거시 모두 같은 파이프라인 합류), 스킬 간 데이터 계약을 정리한 **운영 가이드**입니다.
