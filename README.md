...
### 하네스 설치 후 권장 — 프로젝트 최상위 폴더에 git 계정 설정

`harness-setup`으로 스킬을 배포한 뒤, 프로젝트 최상위 폴더에서 배포본 [`git-scoped-account`](./skills/git-scoped-account/SKILL.md) 스킬로 GitHub·GitLab·Gitea 등 형상관리에 사용할 git 계정(`user.name`/`user.email`)을 설정하는 것을 권장한다. 예를 들어 프로젝트 최상위 폴더가 `acro/`라면 `acro/` 위치에서 실행해, `acro/` 자체는 git 관리 대상으로 보지 않고 바로 아래 애플리케이션 git repo들에 같은 계정을 적용한다. 전역 `~/.gitconfig`는 건드리지 않고 지정한 폴더 바로 아래 git repo들에만 `include.path` 방식으로 공통 계정을 일괄 적용한다.

```text
# 예: acro/ 위치에서, harness-setup으로 배포된 스킬을 사용
@.agents/skills/git-scoped-account/SKILL.md 이 프로젝트 최상위 폴더와 바로 아래 repo들 git 계정 한 번에 설정해줘
```

**왜 하네스 설치 직후에 하는가:** `commit` 등 일부 스킬의 산출물(커밋 작성자, 문서 작성자 표기 등)이 git 계정·사용자 이름을 입력값으로 사용하도록 되어 있다. 프로젝트 최상위 폴더에서 git 계정을 미리 정해 두면, 이후 하네스 산출물이 일관된 작성자 정보로 생성·커밋되어 프로젝트마다 계정이 섞이는 문제를 예방할 수 있다. 특히 한 사용자가 GitHub·GitLab·Gitea 등 저장소별로 다른 계정을 쓰는 환경에서는 이 디렉토리 트리에만 맞는 계정을 먼저 고정해 두는 것이 안전하다.
...
[1단계 — 설계]
        ...
        │
        ▼
/design-doc           (신규 프로젝트 경로 / 주요 사실·흐름 변경 시 갱신 — /doc-audit 경유)
...
/impl-reuse-scan      (구현 시작 전 점검 — 중복 자산 확인)
        ...
        ▼
[3단계 — 구현·검증]
실제 구현 (UI 비중이 높으면 /frontend-design 기준 적용)
        │
        ▼
...
/multi-review + /doc-audit
        │
        ▼
/pre-commit → (선택) /code-comment → /commit
        │
...
- `/harness-bootstrap`은 문서 없는 기존 코드베이스에 최초 진입할 때만 사용한다. 산출물은 `/design-doc`·`/context-doc`와 동일하므로 이후 갱신은 일반 경로를 따른다.

> 흐름 상세, 산출물 매핑, 구현 지침 4종 선택 기준은 [`Harness_Engineering.md`](./Docs/Harness_Engineering.md) §5–§8 참조.
...
| `git-scoped-account` | `이 폴더 아래 repo들 git 계정 한 번에 바꿔줘`, `전역 설정 안 건드리고 하위 repo 계정 일괄 적용` | 전역 `~/.gitconfig`를 건드리지 않고 git으로 관리하지 않는 지정 폴더 바로 아래 애플리케이션 git repo들에 공통 계정(user.name/email)을 `include.path` 방식으로 일괄 적용·확인 |
